...
class UploadQRPresenterImplTest {
    ...
    fun `onActivityResult result cancel, do nothing`() {
        ...
        val data = null
        verify(callbackManager).sendRestoreEvent(RESTORE_UPLOAD_QR_CODE_PAGE_VIEWED)
        whenever(fileSharingHelper.extractUriFromActivityResult(req, res, data)).thenReturn(FileSharingHelper.RequestFileResult(REQUEST_RESULT_CANCELED, null))
    ...
    fun `onActivityResult result failed, show error dialog`() {
        ...
        val data = null
        verify(callbackManager).sendRestoreEvent(RESTORE_UPLOAD_QR_CODE_PAGE_VIEWED)
        whenever(fileSharingHelper.extractUriFromActivityResult(req, res, data)).thenReturn(FileSharingHelper.RequestFileResult(REQUEST_RESULT_FAILED, null))
        presenter.onActivityResult(req, res, data)
